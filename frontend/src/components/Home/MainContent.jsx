...
  
  // Dokunma başlangıcı (Touch Start)
  const handleTouchStart = (e) => {
    // Mobil zoom ve kaydırmayı engellemiyoruz, sadece pozisyonu alıyoruz.
...
    
    // Sürüklemeyi hemen başlatmak yerine, hareket eşiğini bekliyoruz.
    setIsDragging(false); 
  };

  // Dokunma hareketi (Touch Move)
...
      
      // Yeterli sürükleme yapıldıysa veya zaten sürükleniyorsak
      if (Math.sqrt(dx * dx + dy * dy) > 10 || isDragging) {
        
        // Tarayıcının varsayılan hareketini (kaydırma/zoom) engelle
        // Bu, zoom'u engellemenin anahtarıdır.
        e.preventDefault(); 
        
...
    
    // Dokunma olayları
    // 'passive: false' özelliği, 'e.preventDefault()' çağrısına izin verir ve kaydırmayı/zoom'u engellemede kritiktir.
...
    // Sürükleme işlemi bittiğinden emin ol
    if (isDragging) return; 
    
...
      altKategori: selectedCategory === "Market" ? values.altKategori : "", // Form values'ten alıyoruz
      not: values.not || "",
    };
    harcamaMutation.mutate(harcamaData);
  };
  
  // selectedMarket state'i sadece select bileşeninin kendisi için tutuluyordu. 
  // Formun onFinish'i artık `values.altKategori`'yi alacak.
  
...
            <Form.Item 
                name="altKategori" 
                label="Market Seç" 
                initialValue={selectedMarket || MARKETLER[0]} // İlk marketi varsayılan olarak seçin
            >
              <Select
