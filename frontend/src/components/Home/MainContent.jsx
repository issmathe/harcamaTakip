...
const MARKETLER = [
  ...
  "Kaufland", "Norma", "Edeka", "Tegut", "Hit",
  "Famila", "Nahkauf", "Biomarkt", "Penny",
  "Rossmann", "Real", "Diğer",
];
...
// 🚀 Harcamaları API'den çekme
const fetchHarcamalar = async () => {
  const res = await axios.get(`${API_URL}/harcamalar`);
  return res.data;
};
...
const MainContent = ({ radius = 40, center = 50 }) => {
  ...
  // 🧠 Mutation: Gelir ekleme
  const addGelir = useMutation({
    mutationFn: async (gelirData) => {
  ...
  const handleTouchStart = (e) => {
    ...
    const cx = rect.left + rect.width / 2;
    const cy = rect.top + rect.height / 2;
    setLastAngle(getAngle(cx, cy, touch.clientX, touch.clientY));
  ...
  const onHarcamaFinish = (values) => {
    ...
    addHarcama.mutate(harcamaData);
  };

  const onGelirFinish = (values) => {
    addGelir.mutate(values);
  };
