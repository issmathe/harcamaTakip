...
export const useTotals = () => {
  ...
  const fetchTotals = async () => {
    try {
      ...
      setTotalIncome(
        gelirler.reduce((sum, i) => sum + Number(i.miktar || 0), 0)
      );
      setTotalExpense(
        harcamalar.reduce((sum, i) => sum + Number(i.miktar || 0), 0)
      );
...
const MainContent = ({ radius = 30, center = 50, onCategoryClick }) => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      // Başarılı bildirim
      message.success(
        `'${selectedCategory}' kategorisine ${values.miktar} ₺ harcama başarıyla eklendi!`
      );
  ...
  return (
    <main className="flex-1 px-4 pt-4 pb-24">
        ...
        >
          {showIcons ? (
            <>
              ...
              <Text className="block !text-white text-xs mt-1 opacity-80">
                Vazgeç
              </Text>
            </>
          ) : (
      ...
      <Modal
        ...
        open={isModalVisible} // ⬅️ 'visible' yerine 'open' kullanıldı
