...
const Header = () => {
  ...
  const totalSavings = useMemo(() => {
    const gelirTasarruf = (gelirler || [])
      .filter(
        (g) =>
          g.kategori?.toLowerCase() === "tasarruf" ||
          g.kategori?.toLowerCase() === "birikim"
      )
      .reduce(
        (sum, g) => sum + Number(g.miktar || 0),
        0
      );
    ...
    const harcenanTasarruf = (harcamalar || [])
      .filter(
        (h) =>
          h.harcamaKaynagi === "Birikim" ||
          h.kategori?.toLowerCase() === "tasarruf"
      )
      .reduce(
        (sum, h) => sum + Number(h.miktar || 0),
        0
      );
  ...
  const lastMonthData = useMemo(() => {
      ...
      return (list || [])
        .filter((item) => {
          ...
          const isCorrectMonth =
            d.isSame(lastMonth, "month") &&
            d.isSame(lastMonth, "year");

          const isNotTasarruf = isExpense
            ? item.kategori?.toLowerCase() !== "tasarruf"
            : true;

          const isGelir = !isExpense
            ? item.kategori?.toLowerCase() === "gelir"
            : true;

          const isWithinDay = untilToday
            ? d.date() <= todayNum
            : true;
    ...
    return {
      ...
      balance:
        filterByDate(gelirler, false) -
        filterByDate(harcamalar, true)
    };
  }, [harcamalar, gelirler]);
  ...
  const StatBox = ({
    ...
    if (isActive) {
      if (id === "gider") {
        ...
        bgColor =
          "bg-indigo-900 border-indigo-950 scale-95 shadow-inner text-white";
      } else {
        displayVal = prevVal;
        ...
        bgColor =
          "bg-gray-800 border-gray-900 scale-95 shadow-inner text-white";
      }
    }
    ...
    return (
      ...
      >
        <div className="flex justify-between items-start">
          ...
          {isActive &&
            id === "gider" && (
              <CalendarOutlined className="text-[10px] text-indigo-300" />
            )}
        </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                <Button
                  ...
                  size="small"
                  icon={
                    <FormOutlined className="text-white/70 hover:text-white" />
                  }
                  onClick={() =>
                    navigate("/notlar")
                  }
              ...
              <Title
                level={2}
                className="!text-white !m-0 !text-3xl font-black italic"
              >
                €{formatCurrency(
                  totalAssets
                )}
      ...
      <div className="flex gap-2 h-20">
        ...
        <div className="flex-1 bg-orange-50 border-l-4 border-orange-500 rounded-xl p-2 flex flex-col justify-between shadow-sm">
          ...
          <div className="flex justify-between items-center">
            ...
            <Text className="text-orange-700 text-[10px] font-bold uppercase">
              Bugün
            </Text>
