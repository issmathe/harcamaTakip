...
import { Card, Typography, Empty, Button, ConfigProvider } from "antd";
import { 
  ArrowLeftOutlined, 
  ArrowRightOutlined, 
  BarChartOutlined,
...
const categoryColors = {
  "Giyim": "#FF6384", "Petrol": "#FFCE56", "Kira": "#4BC0C0",
...
const RaporlarContent = () => {
  ...
  const [activeTab, setActiveTab] = useState(null); // Trend için başlangıç null
  ...
  const globalMax = useMemo(() => {
    ...
    return Math.max(...values, 100) * 1.2;
  }, [harcamalar]);
  ...
  const renderTabContent = () => {
    if (activeTab === null) {
      return (
        <Card className="rounded-2xl shadow-sm border-none bg-white overflow-hidden mb-4">
          ...
          {trendCalculation.data.some(v => v > 0) ? (
            <div style={{ height: "180px" }}>
              <Line ref={lineChartRef} data={lineChartData} options={{
                responsive: true, maintainAspectRatio: false,
                plugins: { legend: { display: false }, datalabels: { align: "top", offset: 10, formatter: (v) => `${v}€`, font: { weight: "bold", size: 10 }, color: "#1e40af" } },
                scales: { y: { display: false, beginAtZero: true }, x: { grid: { display: false }, ticks: { font: { size: 11 }, color: "#94a3b8" } } },
    ...
    if (filteredHarcamalar.length === 0) return <div className="bg-white rounded-3xl p-12 text-center shadow-sm"><Empty description="Veri Yok" /></div>;
          ...
          <div className="space-y-4">
            <Card title={<Text strong className="text-[11px] uppercase text-gray-400">Giyim Detay</Text>} className="rounded-3xl border-none shadow-sm" bodyStyle={{ padding: '12px' }}>
              <div style={{ height: '160px' }}>
                <Bar data={{ labels: GIYIM_KISILERI, datasets: [{ data: giyimData, backgroundColor: "#FF6384", borderRadius: 4, barThickness: 12 }] }} options={getBarOptions(globalMax)} />
              </div>
            </Card>
            <Card title={<Text strong className="text-[11px] uppercase text-gray-400">Aile Detay</Text>} className="rounded-3xl border-none shadow-sm" bodyStyle={{ padding: '12px' }}>
              <div style={{ height: '130px' }}>
                <Bar data={{ labels: AILE_UYELERI, datasets: [{ data: aileData, backgroundColor: "#AF52DE", borderRadius: 4, barThickness: 12 }] }} options={getBarOptions(globalMax)} />
              </div>
            </Card>
          </div>
        );
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-gray-50 pb-24">
      ...
      <div className="p-4 space-y-4">
        {/* Manuel Segmented: Hem görseli bozmaz hem de bas-kapat çalışır */}
        <div className="flex bg-gray-200/50 p-1 rounded-xl gap-1">
          {[
            ...
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-2 py-1.5 text-[13px] font-medium rounded-lg transition-all duration-200 ${
                activeTab === item.id 
